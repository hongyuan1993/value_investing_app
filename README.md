...
- **可调参数：** 通过滑块调整 FCF 增长率、折现率（WACC）、永续增长率、预测年数；预测年数大于 5 时可将**前 5 年**与**第 6 年及以后**的增长率分开设定，结果实时更新。
...
若出现 **429** 且提示 **spending cap / 支出上限**：说明当前 Google Cloud 项目绑定的结算账号已达到你设置的**预算或支出上限**。请在 [Google Cloud 控制台 → 结算](https://console.cloud.google.com/billing) 中提高预算、调整支出上限，或确认项目已关联有效付款方式后再试。
